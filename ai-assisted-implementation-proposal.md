...
flowchart TB
  ...
  subgraph Gates["品質ゲート (Seam 2)"]
    direction LR
    G1["要件定義ゲート<br/>ｺﾝｻﾙﾀﾝﾄ承認"] --> G2["基設計ゲート<br/>SE承認"] --> G3["開発+単体<br/>テストゲート<br/>SE承認"]
  end
...
|------|--------|--------|-------------|-----------|
| 従来型 | 42,000人日 | ¥2.40B–¥2.40B | - | ツールなし |
| Tier A (Claude Code) | 10,800人日 | ¥396M–¥636M | ¥2.00B | ¥30,000/月/ID |
| Tier B (Joule) | 24,600人日 | ¥876M–¥1.42B | ¥1.52B | 未定 (SAP要確認) |
| Tier C (Copilot) | 31,800人日 | ¥1.42B–¥1.84B | ¥984M | ¥3,000/月/ID |

### フェーズ別内訳（1本あたり）

| フェーズ | 担当 | 従来 | Tier A | Tier B | Tier C |
|----------|------|------|--------|--------|--------|
| 要件定義 | ｺﾝｻﾙ | 10日(¥75万) | 2–3日(¥15万–¥23万) | 4–6日(¥30万–¥45万) | 6–8日(¥45万–¥60万) |
| 基設計 | SE | 5日(¥25万) | 0.5–1日(¥3万–¥5万) | 1.5–2.5日(¥8万–¥13万) | 2.5–3.5日(¥13万–¥18万) |
| 開発+単体テスト | SE | 20日(¥1M) | 3–5日(¥15万–¥25万) | 7–12日(¥35万–¥60万) | 12–15日(¥60万–¥75万) |
| **合計** | | **35日(¥2M)** | **5.5–9日(¥33万–¥53万)** | **12.5–20.5日(¥73万–¥1M)** | **20.5–26.5日(¥1M–¥2M)** |

---

### 見積もり計算方式

本見積もりはすべて以下の計算式に基づきます。

#### 基本計算式

```
総コスト = Σ ( フェーズ別工数 × 単価 × I/F数 ) + ツールライセンス費用

フェーズ別AI工数 = 従来工数 × (1 − 圧縮率)
```
...
|------|-----|
| 対象 I/F 数 | 1,200 本 |
| コンサルタント単価 | ¥1,500,000/月（¥75,000/日 = 月20営業日換算） |
| SE単価 | ¥1,000,000/月（¥50,000/日 = 月20営業日換算） |
| 対象フェーズ（請求対象） | 要件定義 + 基設計 + 開発+単体テスト（35人日/I/F） |
| 対象外（顧客負担） | 結合テスト + UAT（30人日/I/F） |

#### フェーズ別 従来工数と費用（1本あたり）

| フェーズ | 担当 | 工数 | 単価 | 費用計算式 | 費用 |
|----------|------|------|------|-----------|------|
| 要件定義 | ｺﾝｻﾙﾀﾝﾄ | 10日 | ¥75,000/日 | 10日 × ¥75,000 | ¥75万 |
| 基設計 | SE | 5日 | ¥50,000/日 | 5日 × ¥50,000 | ¥25万 |
| 開発+単体テスト | SE | 20日 | ¥50,000/日 | 20日 × ¥50,000 | ¥1M |
| **合計** | | **35日** | | **35日 × 加重平均** | **¥2M** |

#### AI圧縮率と計算式

| 方式 | 要件定義 工数 | 基設計 工数 | 開発+単体テスト 工数 | 計算式（1本あたり） |
|------|-------------|-----------|-------------------|-------------------|
| 従来型 | 10–10日 | 5–5日 | 20–20日 | (10×¥75,000 + 5×¥50,000 + 20×¥50,000) ~ (10×¥75,000 + 5×¥50,000 + 20×¥50,000) |
| Tier A (Claude Code) | 2–3日 | 0.5–1日 | 3–5日 | (2×¥75,000 + 0.5×¥50,000 + 3×¥50,000) ~ (3×¥75,000 + 1×¥50,000 + 5×¥50,000) |
| Tier B (Joule) | 4–6日 | 1.5–2.5日 | 7–12日 | (4×¥75,000 + 1.5×¥50,000 + 7×¥50,000) ~ (6×¥75,000 + 2.5×¥50,000 + 12×¥50,000) |
| Tier C (Copilot) | 6–8日 | 2.5–3.5日 | 12–15日 | (6×¥75,000 + 2.5×¥50,000 + 12×¥50,000) ~ (8×¥75,000 + 3.5×¥50,000 + 15×¥50,000) |

#### 計算例: Tier A (Claude Code) の総額算出

```
【1本あたり AI工数】
  要件定義:      10日 × (1 − 0.70) = 2–3日（70–80%圧縮）
  基設計:         5日 × (1 − 0.85) = 0.5–1日（80–90%圧縮）
  開発+単体テスト: 20日 × (1 − 0.80) = 3–5日（75–85%圧縮）

【1本あたり AI費用（下限）】
  2日 × ¥75,000 = ¥150,000
  0.5日 × ¥50,000 = ¥25,000
  3日 × ¥50,000 = ¥150,000
  小計: ¥325,000 → 10,000円単位丸め: ¥33万

【1本あたり AI費用（上限）】
  3日 × ¥75,000 = ¥225,000
  1日 × ¥50,000 = ¥50,000
  5日 × ¥50,000 = ¥250,000
  小計: ¥525,000 → 10,000円単位丸め: ¥53万
...
【削減効果】
  従来総額 ¥2.40B − Tier A下限 ¥396M = ¥2.00B
  削減率: 84%
```
...
| 4 | 本格採用AIツールの決定と全量展開 | Tier A (Claude Code) | 基設計フェーズ開始時（要件定義完了後） | 採用AIツールが決定済み。全SEのライセンス発行完了。フェーズ別AIワークフロー... |
