...
flowchart TB
  ...
  subgraph BAPI[BAPI_SALESORDER_CREATEFROMDAT2]
    direction TB
...
**データフロー説明**:
1. Salesforce から受注入力データ（ヘッダ 12 項目 + 明細 7 項目）が送信される
2. データ変換層で日付形式・コード変換・桁揃えを実施
3. `BAPI_SALESORDER_CREATEFROMDAT2` を RFC 同期呼び出し
4. SAP 側で VBAK（ヘッダ）・VBAP（明細）・VBPA（パートナ）・VBKD（販売）にデータ登録
5. 成功時は受注伝票番号（SALESDOCUMENT）を返却、エラー時は ZSD_ERR_QUEUE に格納し ROLLBACK
